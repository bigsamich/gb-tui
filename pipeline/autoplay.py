...
def run(state_path: str, model: str, url: str, steps: int, tag: str):
    ...
    for step in range(steps):
        ...
        # "Walk to where I already am" in the INTERIOR = the model wants to ENGAGE what's
        # here (e.g. navigated to Oak at (5,4) but keeps re-issuing walk_to). Convert that
        # to interact. BUT at a map EDGE a self-walk_to is a CROSSING attempt (executor's
        # walk_to steps off the edge to the connected map) -- never convert those.
        if act.get("action") == "walk_to" and (act.get("x"), act.get("y")) == (s["x"], s["y"]):
            gg = NAV.grid(s["map"])
            at_edge = bool(gg) and (s["x"] <= 0 or s["y"] <= 0
                                    or s["x"] >= gg[1] - 1 or s["y"] >= gg[2] - 1)
            if not at_edge:
                act = {"action": "interact"}
        res = emu.do(act, s)
