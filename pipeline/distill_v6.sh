#!/usr/bin/env bash
# v6 distill chain, Q4 ONLY (per request -- skip the q8 quant to save time).
# train LoRA on data/v6 (v4 + errand demos) -> merge -> q4_K_M gguf -> import as
# pokered-8b-v6-q4. Runs unattended; every stage logged with a marker.
set -euo pipefail
cd "$(dirname "$0")/.."
...
OUT=training/runs/pokered-8b-v6
...
log "STAGE 1/4: train LoRA on data/v6"
$PY pipeline/train_lora.py --data data/v6 --model Qwen/Qwen3-8B --out runs/pokered-8b-v6
...
base, out = "Qwen/Qwen3-8B", "training/runs/pokered-8b-v6"
...
f16="$OUT/.pokered-8b-v6.f16.gguf"; q4="$OUT/pokered-8b-v6.q4_K_M.gguf"
python3 "$LLAMA/convert_hf_to_gguf.py" "$OUT/merged" --outfile "$f16" --outtype f16
"$QUANT" "$f16" "$q4" Q4_K_M
rm -f "$f16"
log "STAGE 3 done: $q4"

log "STAGE 4/4: import into dockerized Ollama as pokered-8b-v6-q4"
docker exec ollama mkdir -p /models
docker cp "$q4" "ollama:/models/$(basename "$q4")"
printf 'FROM /models/%s\nPARAMETER temperature 0.1\nPARAMETER num_ctx 4096\n' "$(basename "$q4")" > "$OUT/Modelfile.v6-q4"
docker cp "$OUT/Modelfile.v6-q4" ollama:/models/Modelfile.v6-q4
docker exec ollama ollama create pokered-8b-v6-q4 -f /models/Modelfile.v6-q4
docker exec ollama ollama list | grep -E "pokered-8b-v6" || { log "ERROR: v6 not imported"; exit 1; }
log "ALL DONE: pokered-8b-v6-q4 ready. Next: eval navigation + errand + starter/dialog."
