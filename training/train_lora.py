...
def main():
    ...
    data_dir = (HERE / args.data) if not Path(args.data).is_absolute() else Path(args.data)
    ...
    trainer.train()
    trainer.save_model(str(HERE / args.out / "adapter"))
    tok.save_pretrained(str(HERE / args.out / "adapter"))
    print("saved adapter ->", HERE / args.out / "adapter")
